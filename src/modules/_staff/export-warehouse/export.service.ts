...
export class ExportWareHouseService extends BaseService<ExportWareHouse> {
  ...
  async checkInExportWarehouse(id: string, user: any){
    console.log(user);
    
    // 007127be-e453-40b6-85b1-384489322a87
    // find the entity
    const entity = await this.exportWarehouseRepository.findOne({
      where: { 
        id: '007127be-e453-40b6-85b1-384489322a87' , status: exportStatus.ACCEPTED
      },
      relations: ['task', 'task.fixer'],
    });
    if(!entity){
      throw new HttpException('Not found or not accepted or exported', 404);
    }
    // check user is fixer
    if(entity.task.fixer.id !== user.id){
      throw new HttpException('You are not fixer', 403);
    }

    return await this.exportWarehouseRepository.update(id, {status: exportStatus.EXPORTED});
