...
export class RequestService extends BaseService<RequestEntity> {
  ...
  async customHeadStaffGetOneRequest(userId: string, id: string): Promise<RequestEntity> {
    ...
    return this.requestRepository.findOne({
      where: { id },
      relations: ['device', 'device.area', 'device.machineModel', 'tasks', 'tasks.fixer', 'requester', 'issues', 'issues.typeError', 'issues.issueSpareParts', 'issues.issueSpareParts.sparePart'],
    });
  }
