...
export class TypeErrorController {
  ...
  @ApiResponse({
    type: TypeErrorResponseDto.TypeErrorGetAll,
    status: 200,
    description: 'Get all TypeErrors available with decive id is null (common)',
  })
  @Get('get-all-common/:id')
  async getAllWithDeviceId() {
    return await this.typeErrorService.getAllTypeErrorByDeviceIdNull();
  }
